...
% Node generator: 'uniform' for reviewer test, 'gaussian' for original paper
nodeGenerator = 'uniform';
...
for mc = 1:nMC
    ...
    switch lower(nodeGenerator)
        case 'uniform'
            [xNodes, sNodes] = generateNodesUniform(N, spot0, xMin, xMax);
        case 'gaussian'
            [xNodes, sNodes] = generateNodesQMC(N, spot0, xMin, xMax);
        otherwise
            error('Unknown nodeGenerator: %s. Use ''uniform'' or ''gaussian''.', nodeGenerator);
    end
...
function [xNodes, sNodes] = generateNodesUniform(N, spot0, xMin, xMax)
% Generate N nodes in log-space x from the uniform distribution on [xMin,xMax]^d.
% First node is fixed at x=0 so that sNodes(:,1)=spot0 and the price is read
% directly at value(1). The remaining N-1 nodes are uniform in the box.
...
if useSobol
    ...
    U = net(p, N-1);            % quasi-uniform points in [0,1]^d
else
    U = rand(N-1, d);           % pseudo-random uniform points in [0,1]^d
end

Y = bsxfun(@plus, xmin.', bsxfun(@times, U, (xmax - xmin).'));
X(:,2:N) = Y.';
